...
## Charts in cluster-env-repo
This section describes the two charts found in cluster-env-repo, Nidhogg and Yggdrasil. 
...
### Step two
Now that the application is created, the developers needs to create a pull request to Yggdrasils values.yaml file. This pull request should add the following code block:
```
projects: 
  - name: myproject
      ...
      apps:
        - name: myappname
          source:
            repoURL: "mygithubURL.git"
            targetRevision: HEAD (or the name of a branch)
            path: the path where your application is located in the repo
```

This PR will then need to be approved by the cluster development team, before it is merged into cluster env repository. When this is merged, ArgoCD will automatically deploy the application onto the cluster. 
